...
|---|---|---|
| Topic subscription (fanout) | `RoutingMode.Broadcast` <br> or explicit subscription: `bus.Subscribe<MyMessageType>(handler)` | Dispatched message is received by all receivers that are subscribed |
| Queue subcription (fifo) | `RoutingMode.Queue` | Dispatched message is received by one subscriber at a time in the order of their registration |
...
> [!NOTE]
> <i>Currently, no dead letter functionality is available, but I may implement this as a sort of fallback measure regardless of drop count being hit.</i>
